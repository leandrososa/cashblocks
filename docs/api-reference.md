...
Indeterminate cash-device outcomes use
...
All implementations also expose `id`, `kind`, and `capabilities`.
`validateTerminalAdapters` checks slot kinds, unique ids, and capability values.
Adapter calls receive an optional `AdapterOperationContext` generated by the
runtime with operation/session correlation, deadline, and an `AbortSignal`.
The runtime enforces the configured timeout and returns `ADAPTER_TIMEOUT`.
Cash dispenser and acceptor timeouts instead return
`ADAPTER_OUTCOME_UNKNOWN` and produce `transaction.reconciliation_required`,
because a cooperative abort cannot prove whether cash moved.
